...
const NowPlaying: React.FC = (): JSX.Element => {
  ...
  return (
    <div className="flex items-center">
      ...
      <div className="flex w-full flex-col flex-wrap justify-center px-2 md:flex-row md:items-center md:justify-start">
        {data?.songUrl ? (
          <a
            href={data.songUrl}
            target="_blank"
          ...
          <p className="text-base font-bold">Not Playing</p>
        )}
        <span className="mx-1 hidden text-base md:block">-</span>
        <p className="p-1 text-base">{data?.artist ?? "Spotify"}</p>
      </div>
    </div>
  );
};
